...
## Cost & identity — important for multi-tenant bots

Like Me Like is free to use: no payment, no card, no account. What
still matters is the id:

- Because the agent ID is per-end-user (you're hashing the phone
  number, Discord user id, etc.), every user you onboard gets their
  own taste profile. One shared id would blend all your users'
  tastes into one profile.
- Fair-use rate limits apply per id.
